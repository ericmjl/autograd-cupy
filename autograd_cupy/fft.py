...
defvjp(
    fft, lambda *args, **kwargs: fft_grad(get_fft_args, fft, *args, **kwargs)
)
defvjp(
    ifft, lambda *args, **kwargs: fft_grad(get_fft_args, ifft, *args, **kwargs)
)

defvjp(
    fft2, lambda *args, **kwargs: fft_grad(get_fft_args, fft2, *args, **kwargs)
)
defvjp(
    ifft2,
...
defvjp(
    fftn, lambda *args, **kwargs: fft_grad(get_fft_args, fftn, *args, **kwargs)
)
defvjp(
    ifftn,
...
defvjp(
    ...
    lambda ans, x, axes=None: lambda g: match_complex(
        x, acp.conj(ifftshift(acp.conj(g), axes))
    ),
)
defvjp(
    ifftshift,
    lambda ans, x, axes=None: lambda g: match_complex(
        x, acp.conj(fftshift(acp.conj(g), axes))
    ),
)
...
defvjp(
    truncate_pad,
    lambda ans, x, shape: lambda g: match_complex(
        x, truncate_pad(g, vspace(x).shape)
    ),
)
